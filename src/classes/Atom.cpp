...
void Atom::applyForces(std::vector<Atom>& others, float dt, const Config& cfg) {
    // Binding forces
    for (auto& e : electrons) {
        glm::vec3 r = glm::vec3(e.getPosition()[0], e.getPosition()[1], e.getPosition()[2]) - nucleus.position;
    ...
    for (auto& other : others) {
        ...
        for (auto& e : electrons) {
            glm::vec3 r = other.nucleus.position - glm::vec3(e.getPosition()[0], e.getPosition()[1], e.getPosition()[2]);
            glm::vec3 F_on_e = coulombForce(r, e.getCharge(), other.nucleus.charge, cfg.coulomb_k, cfg.coulomb_softening);
            std::vector<double> vel_e = e.getVelocity();
    ...
    for (auto& other : others) {
        ...
        glm::vec3 F = coulombForce(r, nucleus.charge, other.nucleus.charge, cfg.coulomb_k, cfg.coulomb_softening);
